// ── SLIDER ──
  ...
  function sendToWhatsApp() {
    const firstName = document.querySelector('input[placeholder="Rahul"]').value.trim();
    const lastName  = document.querySelector('input[placeholder="Sharma"]').value.trim();
    const email     = document.querySelector('input[placeholder="rahul@business.com"]').value.trim();
    const phone     = document.querySelector('input[placeholder="+91 98765 43210"]').value.trim();
    const service   = document.querySelector('.contact-form-box select').value.trim();
